######ROX <=4.88
intermediate.proof<-target.trial.proof%>%group_by(id)%>%
  mutate(invasive_cumsum = cumsum(invasive)) %>% ####by removing observations when cumsum >1 and then filtering by inclusion criteria, we make sure no patients intubated at hour 0 or before are included since we have lagged and there is no information on hour 0
...
intermediate.proof <- intermediate.proof %>%
  group_by(id,hour) %>%
  mutate(exclusion_met = cumsum((SaFi<88|ROX>4.88|RR>39|gcs<12)))%>%ungroup()
...
####947 patients
dim(inclusion.proof) ####roughly 4000 observations
...
dim(ids.24h) ####53000 observations
...
dim(intubated.proof)####733 patients intubated out of 2100 patients
...
explanatory <- names(target.to.impute.proof[,c(10,11,13,20:24,26:29,31,37,47:52)])
...
write_xlsx(eligibility.after%>% 
             summary_factorlist("ever.intubated", explanatory.final, p=TRUE),path ="baseline.table.low.rox.xlsx")
...
KM.plot.truncated<-ggsurvplot(km_model.truncated,data=mimic.to.truncate,pval="Hazard ratio: 0.72, 95% CI 0.52-0.99, p= 0.046",censor=F,risk.table = T,legend.labs=c("Intubation = No", "Intubation = Yes"),title = "Kaplan-Meier Curves by Intubation Group",xlab = "Time",
...
####using average curves
KM.plot.truncated.unstratified<-ggsurvplot(surv_curves,data=mimic.to.truncate,pval="Hazard ratio: 0.72, 95% CI 0.52-0.99, p= 0.0.046",censor=F,risk.table = T,legend.labs=c("Intubation = No", "Intubation = Yes"),title = "Kaplan-Meier Curves by Intubation Group",xlab = "Time",
                                           ylab = "Survival Probability",pval.coord = c(100,0.3))
KM.plot.truncated.unstratified$plot+geom_vline(aes(xintercept = 28), linetype = "dashed", color = "blue") +
  geom_vline(aes(xintercept = 60), linetype = "dashed", color = "red")+scale_x_continuous(breaks = c(0,90,365),expand = c(0,0))
