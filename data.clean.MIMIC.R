...
mimic.target.trial.dod <- read.csv("~/Downloads/mimic.target.trial.dod.csv")
target.trial<-mimic.target.trial.dod
dim(target.trial)
target.trial<-mimic.target.trial.dod<-target.trial<-mimic.target.trial.dod[,-44]
length(unique(target.trial$subject_id)) ####40559 unique patients
