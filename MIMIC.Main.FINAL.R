...
inclusion.proof<-intermediate.final.proof %>% 
  filter(
    hour <= 48,
    SaFi <=200 & spO2<=97 & code_status=="Full code"
  )
...
dim(intubated.proof)####792 patients intubated out of 2996 patients
...
dim(target.unimputed.proof) ####we retain roughly 38000 observations from 2900 patients 
length(unique(target.unimputed.proof$id))
...
eligibility.after%>% 
  summary_factorlist("ever.intubated",explanatory.final, cont_nonpara=10,cont_range=T,p=TRUE)
...
mimic.imputed.final%>%filter(invasive==1)%>%mutate(quantile=quantile(reset.hour))

mimic.imputed.final%>%summary_factorlist("invasive",explanatory.final, cont_nonpara=10,cont_range=T,p=TRUE)
write_xlsx(mimic.imputed.final%>%summary_factorlist("invasive",explanatory.final, cont_nonpara=10,cont_range=T,p=TRUE),path ="baseline.table.mimic.aggregated.xlsx")

dim(eligibility.after)
length(unique(eligibility.after$id))
table(eligibility.after$ever.intubated)
quantile(eligibility.after$hour)
quantile(table(mimic.imputed.final$id))


################## 5TH STEP:IPTW########
mimic.imputed.final.target<-mimic.imputed.final
write.csv(mimic.imputed.final.target, "mimic.imputed.final.csv", row.names = FALSE)
...
predicted_outcomes <- predict(cox_model.truncated, type = "response")

####KM
# Plot Kaplan-Meier curves
KM.plot.truncated<-ggsurvplot(km_model.truncated,pval="Hazard ratio: 0.81, 95% CI 0.68-0.96, p= 0.018",censor=F,risk.table = T,legend.labs=c("Intubation = No", "Intubation = Yes"),title = "Kaplan-Meier Curves by Intubation Group",xlab = "Time",
...
# Fit a weighted Cox proportional hazards model
cox_model.truncated.30d <- coxph(surv_object.truncated.30d ~ invasive+SBP+DBP+MBP+HR+temp+bilirubin_max+creatinine_max+platelet_min,cluster=id,weights=weights,data = mimic.to.truncate,robust=T)
summary(cox_model.truncated.30d)

cox_model.truncated.strata.30 <- coxph(surv_object.truncated.30d ~ strata(invasive) + SBP+DBP+MBP+HR+temp+bilirubin_max+creatinine_max+platelet_min,cluster=id,data = mimic.to.truncate, weights = weights,robust=T)
...
####KM
# Plot Kaplan-Meier curves
KM.plot.truncated.30d<-ggsurvplot(km_model.truncated.30d,data=mimic.to.truncate,pval="Hazard ratio: 0.80, 95% CI 0.64-0.99, p= 0.046",censor=F,risk.table = T,legend.labs=c("Intubation = No", "Intubation = Yes"),title = "Kaplan-Meier Curves by Intubation Group",xlab = "Time",
...
cov2<-c("age","elixhauser","RR","SaFi","fio2.final","gcs","vasopressor","first_careunit")
aipw.mortality.lr<-AIPW$new(Y=as.numeric(mimic.to.truncate$mortality.1y.censored)-1,A=as.numeric(mimic.to.truncate$invasive)-1,W.Q=subset(mimic.to.truncate,select=cov),W.g=subset(mimic.to.truncate,select=cov2),Q.SL.library="SL.glm",g.SL.library="SL.glm",verbose=T,k_split=10)
...
bootstrap_stats <- function(data, n_bootstrap=1000) {
  replicate(n_bootstrap, {
    ...
    stats_group1 <- sampled_data %>% filter(invasive == 0) %>% summarize(median = weightedMedian(ICU_los,weights=weights,na.rm=T), q25 = weightedQuantile(ICU_los, weights=weights,0.25,na.rm=T), q75 = weightedQuantile(ICU_los, weights=weights,0.75,na.rm=T))
    stats_group2 <- sampled_data %>% filter(invasive == 1) %>% summarize(median = weightedMedian(ICU_los,weights=weights,na.rm=T), q25 = weightedQuantile(ICU_los, weights=weights,0.25,na.rm=T), q75 = weightedQuantile(ICU_los, weights=weights,0.75,na.rm=T))
...
set.seed(123)
boot_results <- bootstrap_stats(mimic.to.truncate)
...
bootstrap_stats <- function(data, n_bootstrap = 1000) {
  replicate(n_bootstrap, {
    ...
    # Calculate medians and quantiles for each group
    stats_group1 <- sampled_data %>% filter(invasive == 0) %>% summarize(
      median = weightedMedian(ICU_los, weights = weights, na.rm = TRUE),
      q25 = weightedQuantile(ICU_los, weights = weights, 0.25, na.rm = TRUE),
      q75 = weightedQuantile(ICU_los, weights = weights, 0.75, na.rm = TRUE)
    )
    
    stats_group2 <- sampled_data %>% filter(invasive == 1) %>% summarize(
      median = weightedMedian(ICU_los, weights = weights, na.rm = TRUE),
      q25 = weightedQuantile(ICU_los, weights = weights, 0.25, na.rm = TRUE),
      q75 = weightedQuantile(ICU_los, weights = weights, 0.75, na.rm = TRUE)
    )
...
####Hosp_LOS###########
####convert to days
mimic.to.truncate$HOSP_los<-mimic.to.truncate$HOSP_los/24
...
bootstrap_stats <- function(data, n_bootstrap=1000) {
  replicate(n_bootstrap, {
    ...
    # Calculate medians and quantiles for each group
    stats_group1 <- sampled_data %>% filter(invasive == 0) %>% summarize(median = weightedMedian(HOSP_los,weights=weights,na.rm=T), q25 = weightedQuantile(HOSP_los, weights=weights,0.25,na.rm=T), q75 = weightedQuantile(HOSP_los, weights=weights,0.75,na.rm=T))
    stats_group2 <- sampled_data %>% filter(invasive == 1) %>% summarize(median = weightedMedian(HOSP_los,weights=weights,na.rm=T), q25 = weightedQuantile(HOSP_los, weights=weights,0.25,na.rm=T), q75 = weightedQuantile(HOSP_los, weights=weights,0.75,na.rm=T))
