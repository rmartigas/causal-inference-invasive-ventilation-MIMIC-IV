...
intermediate.proof <- intermediate.proof %>%
  group_by(id,hour) %>%
  mutate(exclusion_met = cumsum((SaFi<88|SaFi>150|RR>39|gcs<12)))%>%ungroup()
...
dim(intubated.proof)####308 patients intubated out of 2100 patients
...
write_xlsx(eligibility.after%>% 
             summary_factorlist("ever.intubated", explanatory.final, p=TRUE),path ="baseline.table.low.safi.xlsx")
...
KM.plot.truncated<-ggsurvplot(km_model.truncated,data=mimic.to.truncate,pval="Hazard ratio: 0.96, 95% CI 0.71-1.31, p= 0.81",censor=F,risk.table = T,legend.labs=c("Intubation = No", "Intubation = Yes"),title = "Kaplan-Meier Curves by Intubation Group",xlab = "Time",
...
mimic.to.truncate$ever.intubated
target.trial<-mimic.target.trial.dod.minus1 
